...
public class BookService {
    ...
    public void updateAuthorV1(int id,Author author){
        Author exitingAuthor = authorDao.findById(id).orElseThrow(EntityNotFoundException::new);
        exitingAuthor.setBooks(author.getBooks());
       // exitingAuthor.setId(author.getId());
        exitingAuthor.setName(author.getName());
        exitingAuthor.setAddress(author.getAddress());
        exitingAuthor.setPhoneNumber(author.getPhoneNumber());
        exitingAuthor.setDateOfBirth(author.getDateOfBirth());
    }
}
